...
const combineDateTime = (date, time) => {
  ...
  const result = new Date(`${date}T${time24}:00`);
...
const normalizeVehicle = (vehicle, index) => {
  ...
  const fallbackImages = [car1, car2, car3, car4, car5, car6];

  const rawImage =
    Array.isArray(vehicle.images) && vehicle.images.length > 0
      ? vehicle.images[0]
      : vehicle.image || vehicle.vehicleImage || "";
  ...
  const transmission = vehicle.transmission || vehicle.gearType || "Automatic";

  const fuel = vehicle.fuel || vehicle.fuelType || "Diesel";

  const seats =
    vehicle.seats || vehicle.seatingCapacity || vehicle.capacity || "7 seats";
  ...
  return {
    ...
    rating: vehicle.rating || vehicle.averageRating || "4.96",
  ...
  const fetchVehicles = async () => {
    try {
      ...
      if (Array.isArray(response.data?.data)) {
        ...
        vehicleData = response.data.listings;
      ...
      const mostSearchedVehicles = vehicleData
        .filter((vehicle) => {
          ...
          return listingType === "most searched cars" && status === "active";
        })
        .sort((a, b) => {
      ...
      const normalized = mostSearchedVehicles
      ...
      console.error("FETCH MOST SEARCHED VEHICLES ERROR:", error);

      console.error("SERVER RESPONSE:", error?.response?.data);

      // Don't show unrelated dummy cars
      setVehicles([]);
    } finally {
...
const fetchLocations = async () => {
  try {
    ...
    const normalizedLocations = locationData
      .filter((location) => {
        ...
        return String(location.status).toLowerCase() === "active";
      })
      .map((location) => ({
        ...location,
    ...
    if (normalizedLocations.length > 0) {
      const firstLocation = normalizedLocations[0].name;

      setFormData((previous) => ({
        ...previous,
        pickupLocation:
          previous.pickupLocation || firstLocation,
        dropoffLocation:
          previous.dropoffLocation || firstLocation,
      }));
    }
  } catch (error) {
  ...
  const handleOpenModal = (car) => {
    ...
    const matchingLocation = locations.find(
      (location) =>
        ...
        String(car.location || "").toLowerCase(),
    );

    const pickupLocation = matchingLocation?.name || defaultLocation;
    ...
    setFormData((previous) => ({
      ...
      dropoffLocation: previous.dropoffLocation || defaultLocation,

      pickupDate: previous.pickupDate || getTodayDate(),

      dropoffDate: previous.dropoffDate || getTomorrowDate(),
    }));
  };
  ...
  const handleConfirmBooking = async (e) => {
    ...
    const pickupDateObject = new Date(formData.pickupDate);

    const dropoffDateObject = new Date(formData.dropoffDate);
    ...
    if (!dropoffDateTime) {
      alert("Invalid drop-off date/time.");
    ...
    const amount = Number(selectedVehicle.price) || 0;
      ...
      <div className="most-vehicles-container">
        ...
        <div className="vehicles-grid" ref={scrollContainerRef}>
          {loadingVehicles ? (
            ...
            vehicles.map((car) => (
              <div className="vehicle-card" key={car.id || car.name}>
                {/* Image Wrapper */}
                ...
                <div className="card-content">
                  ...
                  {/* Short Description */}
                  {car.shortDesc && (
                    <p className="vehicle-short-desc">{car.shortDesc}</p>
                  )}
        ...
        <div className="modal-backdrop" onClick={handleCloseModal}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-left">
              ...
              {(selectedVehicle.shortDesc || selectedVehicle.fullDesc) && (
              ...
              >
                ...
                <div className="form-group">
                  ...
                  <div className="input-with-icon select-wrapper">
                    ...
                    >
                      ...
                      {locations.map((location, index) => (
                        <option
                          key={location.id || `${location.name}-drop-${index}`}
                          value={location.name}
                        >
                          {location.name}
                        </option>
                ...
                <div className="form-actions">
                  ...
                  <button
                    ...
                    disabled={
                      bookingLoading || loadingVehicles || loadingLocations
                    }
                  >
                    {bookingLoading ? (
                      "Submitting..."
